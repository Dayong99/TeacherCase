  ...
  <div>
    ...
    >
      ...
      >
        ...
        <el-table-column label="作业权重(%)" align="center" width="140px">
          <template slot-scope="scope">
            {{ scope.row.weight }}
          </template>
        </el-table-column>
...
import HomeworkDetail from "./View";
  ...
  methods: {
    ...
    fetch(params = {}) {
      ...
      this.$get("caseteach/job/caseStuJobs", {
        ...
        casename: this.nowCase.casename,
        userid: this.userid,
        sta: 1,
      }).then((r) => {
        if (r.data.code == 800) {
    ...
    async getPdf(row) {
      await this.$get("caseteach/group-stu/backGroupId", {
        ...
        .then((data) => {
          if (this.groupId) {
            this.$get("caseteach/job/group_correct", {
              jobid: row.id,
