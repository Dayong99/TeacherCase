  ...
  <div v-show="tabShow">
    ...
    >
      ...
      >
        <template slot-scope="scope">
          <span>{{
            scope.row.fid ? scope.row.fname : scope.row.sectionname
          }}</span>
        </template>
      </el-table-column>
      <el-table-column
      ...
      >
        <template slot-scope="scope">
          <span>{{ scope.row.fid ? scope.row.sectionname : "" }}</span>
        </template>
      </el-table-column>
    ...
    <div class="dialog-footer">
      <el-button type="warning" plain @click="closeHomework">{{
        $t("common.cancel")
      }}</el-button>
...
export default {
    ...
    isVisible(val) {
      if (val) {
        if (this.caseList.length !== 0) {
          this.queryParams.casename = this.caseList[0].caseName;
        }
      }
    }
  },
  ...
  methods: {
    ...
    getChildName(id) {
      if (id) {
        return new Promise((resolve, reject) => {
          this.$get("caseteach/job/selectChildName/" + id).then(r => {
            resolve(r);
          });
        });
      }
    ...
    fetch(params = {}) {
      this.loading = true;
      this.$get("caseteach/job/caseStuJobs", {
    ...
    add() {
      ...
      this.$refs.edit.setAddValue();
      this.$refs.edit.getCase(this.caseList);
      this.disabled = false;
    },
    edit(row) {
      this.$refs.edit.getCase(this.caseList);
    ...
    delete(sectionIds) {
      ...
      sectionIds.forEach(e => {
        this.$post("caseteach/job/deljob", {
          jobid: e
        }).then(() => {
          this.$message({
    ...
    correct(row) {
      ...
      if (total > 100) {
        ...
        this.$jsonPost("caseteach/job/upd_weight", {
          stu: arr
        }).then(() => {
