<template>
  <div>
    <el-dialog :width="width" :visible.sync="isVisible">
      ...
      >
        ...
        <el-table-column label="班级" align="center">
          <template>
            <span>{{ teachingInfo ? teachingInfo.className : "" }}</span>
          </template>
        </el-table-column>
    ...
    <!--成绩详情 -->
    <detail ref="detail" :dialog-visible="detailVisible" @update="fetch" @close="detailClose" />
  </div>
</template>
<script>
...
export default {
  ...
  methods: {
    ...
    fetch(params = {}) {
      ...
      this.loading = true;
      this.$get("caseteach/teachjob/getAllStu", { ...params }).then(r => {
    ...
    downloadUrl() {
      if (this.teachingInfo) {
        return (
          globalConfig.baseURL +
          "caseteach/teachjob?teachid=" +
